# SEGUNDA VUELTA

Esta vuelta es doned agregamos cosas de la vida rutinarias que quermos mejor o queremos quremos agregar, bueno seria dividir en doa areas las que ya sabemos que hacmeos todos lso dias y las que nos gutaria hacer.

Pongo el ejemplo de alguien que hace mucho deporte entonces sus tecnias las tiene aprendidas mejor que otro pero capaz o que fallas es el estudio entonces deberia aprender tecnicas de estudio al igual qe aprendio las tencias de fueraz fisisca.

Por otro lado una preosna que sabe muchos de estudios capaz quiere saber mas de fuerza fisica y aprender las tecnicas de forma metodocia entonces tambiens estameotodlogia le ayuda porque todo va de la mno, lo que quieras aprender lo avs a tener qe aplicar y lo qe y sabes lo puedes tracker entonces asi es mejor saber el verdaero pasoa  apso de tus habilideases y por awarnnesss seguir mejorando con el flow.

## ALGO FACIL PRIMERO

Ahora viene la parte en la que daptas esta metodolgoia a tu vida diaria, es decir esta bine tener algo para el trabajo pero esta mejor ver como tu vida diaria impacta en tu eficiencia. Hay dias en los que la felicidad es rebosante y entocnes podemos no querer trabajar nada jaa pero otros dias como de depresion vemos solo salida tras el trabajo entonces estos estados de animo se reflejan en tus habitos de dia a dia. Si eres una persona deportiva seguro que no te cuesta hacer mucho ejercio pero capaz estudiar no sea lo tuyo. Es un gran ejepmplo de que tu profesion debe ir incluida de tus habitos. No todos los dias hacemos algo que deberiamos ahcer y haacemos algo que no deberiamos. Esta condicion humana se supera porque tenemos motiviaicon y aaveces al neceseidad de terminar un proyecto saca de nosotros otro tipo de energia. espor esto que si tenemos que organizar nuestro racticad liberada debemos entender aunque sea de forma relacionada en promedio que afectos de nuestra vida privada esta desarrollando en nuestra productividad un caso de obstaculo. Por suerte esta interrelacion es igual que el anterior. Bueno igual no es pero muy parecido en esencia, veamos los deatlles.

Primero empecemos entendiendo que lo que ahremos ahora es interconectar nuestros habitos en la metodologia a manera de registrar como afectan las practicas deliberdas por esto tambien incluye crear un proyecto. pues necesitamos una base donde tener todo guardado y ademas podemos utilizarla como bitacora de lo que pienso seria algo muy relacionado con los habitos y es la salud. Puesto que queremos mejoras nuestra salud empecemos por debajo es decir solo intento una actividad que nos ayude a mejorar nuestro renidmiento y por otro lado vamos a reloctar datos de estos habitos. Vemaos pues como empezza algo asi. Como nombre teendra bitacora de nuestra salud tamto fisica como mental.

### CREAR UN PROYECTO PARA LA SALUD

Bueno ya sabemos como se hace esto primero que anda como tenemos un nuevo proyecto de vida deemos confgirar nuestro cauderno digital pues ahi donde ponemos los nombre clave. PAra este caso utilizaremos la letra J para proyecto J de esta forma siempre qe hablemos de nuestra salud sabemos donde estamos situado nuestra base, igual necesitamos hojas con su indice para sus diferentes draft que vayamso teniendo. Para empezar una hoavamos a crear la mision vision y objetivos de nuestro proyecto como hciismo antes. Una vez definido es imporane que identifiquemos que queremos decir con salud para nuestro fisico entonces estaria bueno hacer un diagram de los diferenes ejercicios que nos ayudaria. Muy posiblmente todos pensamos qe el deporte es salud pero habiendo tantos deporte podriamos quedarnos en que todos pueden ser saludables pero para este caso de configuracion de nuestra metodologia HK he decidido que el ejercicio del ashtanga yoga es el indicado para cauqluier cso que la concentracion sea la meta. Un deportista o un cientifico capaz el hecho de estimular nuestros nervios en cornidinacion con nuestros musculos atravez de la respiracion y el ejercicion de bajo impacto es como un remedio a la enfermedad de vivir planificando sin ejectura, Aveces necesitamos equilibrar los estragos de la vida y cada ma;ana se ha demostrado que tomar un remedio de esta naturaleza trae beneficio tanto fisicios como psicologicos. Capaz no salgas el mas flexible ni el mas fuerte pero mas saludable si osi qe o vas a obtenr.Pues podemos seguir haciendo darft sobre este tema averiguar capz como es y de donde viene. Todo lo qe vyaamos ecnontrado lo vamos escribiendo en forma de tomar notas. que tal ahrao una lista con fotos de la indumentaria. Y ahora una planificacion de nuestro tiempo cuando y donde lo vamos ahcer,

Este ha sido nuevo un paso sencillo porque de nuevo es automatico las cosas donde van en orden y salen y no hay neecesidad de preocuparse por el como solo ejecutar. Con esto podemos tener un nuevo proyecto es decir necesitamos un lugar donde poner en limpio. Seria bueno que como es de salud utilicemos este documento como diario. Escribir sobre nuestra salud de manera periodica detallando lo que estamos haciendo y qe esultados vemos.Para un doctor eso es oro para dar diagnosticos. punto y aparte. Esta el hecho que ya tenemos ela metodologia HK, solo debemos crear un repositior en el github y empezar con un readme, podemos copiar nuestro misiony vision y ademas del diagrama lo podemos convertir en los diferentes ejercicios qe queremos realizar a lo largo de este a;o. Ya asi completamos la creacion de nuestro poryecto para salud.

#### solamente toma rehistro e lo qe pasa

Como veras ahora solo estamos ahciendo uso de las tecnicas aprendais en los anteiores capitlos para la creacion de un nueov proyecto con ideas y tomando notas es parte del planificar con draft y el hechode tener un repositiro donde escribir a diario es la enica de escriibir un diario. Osea este proyecto de solo aparecer ya esta haceindo uso de dos tecnicas y este es el nivel de detalle qe toma en ceunta HK es decir que podemos identiifcar caunto t iempo te tomo desde la planificacion hasta la ejecucion sin necesidad de afectar tu flow, Por eso la esencia de esta metodologia es tomar registro de lo qe ahces dia a dia identificando qe utlizas tencias comunes todo el timepo y midiendoleas a base de una integracoin de varias aplicaicones y apasos del circuito de la metodologia HK.

Por le momento no estamos empezando nada solo confiurando nuestro HK por lo qe no es neceario trackaear esta parte ya que es esencial primero dejar todo progrmado para qe no afecte el flow de concentracion. En este momento a pesar de tener ya tecncias por registrar vmaos a enfocarnos en cofnigurar la metodologia HK.

#### es un diario de tu regeneracion

En pocas palabras lo que hicismo al hacer esta paso fue crear una libro que habla sobre la regeneracion y si ay sos viejo como yo entonces habla de tu intento de ser mas saluble ocmo antes y sino es tu oportunidad para ir rgistrando tu salud a tal nivel qe te vuelva expertos de lo que te hace bien y lo que te crea mas bien un obstaculo, Ademas de deciarte de forma conciente a tu salud lo cual solo puede traer beneficios porque la princpal fuente de cosas danias para nuestro cueerpo somos nosotros mismo por lo que ver como nos hacemos da;o ta solo por no hacer cosas que nos regenren la salud es una alerta para lo que srive este ldiaro qe creamos.

De esta manera no queremos volvernos expertos en estosinoqe es una entrada muy comun entre todos donde nos va explicar a travez de este capitulo como la metodologia integra habitos con practicas deliberadas relacionadlas y dandondels una causalidad.

Por eso el nombre y su creacion es un incio poruqe mal qe mal todos debemos tener esto en nuestras vidas y casualmente es lo que nos explica de forma intutitva las ora faceta de la metodologia HK al darnos las herramientas para resgintrar tambien nuestro progreso en practicas deliberads mas enfocadas a l ejerccio, por como veremos tomar el tiempo de estos eventos no son igual qe otros trabajos. Cpaz detectar como una patada a un balon para mandarla con fuerza necesita de mas control sobre la practica que realizar una website porque aunque las dos son habilidades que se peuden perfeccionar a travez de la practica tiene formas diferentes de tomar su tiempo, Es asi qeu teniendo este nombre acodrnemonos eq sera el proyecto donde utilicemos la metolgia HK apra nuestros habitos.

Es decir si ponerle a un nombre a un proyecto que nsiiquier es prinicpal sino al que dedicaremos unos minutos para indicar como estamos ejectunrado nuestro plan de regenera la salud y como primer intento hemos decidio apliar los ejercios de ashtanga yoa coom remedio al no poder enfocarse por tener llena la metne y el cuerpo duro. En este proyecto vamos a agregar mas remedios a emdida qe vanzamos pero ahora como descurbirmos en nuetrs investigamcione estamosa ntes una practica qeue tiene micso niveles que podemos ir vanzando de apoco por ahora solo nso enfocaqremos el principal ejercico los saludos al sol A y B teniendo asi nuestro pirmer objetivo para este proyecto quizas llegar a 108 saludso ma;anereos y digamos pues que este pryecto al llamrse J tiene un subtitullo de Saludos al sol con esto es suficiente para pasar al siguiente nivel donde continuaremos con este proyecto para idntiicar como la metodolgia HK registra nuestros habitos puesto que al final de cuentas lo que quremos es tener este habito de haecer yoga como remdioa nuestra salud y potenicador de nuestrsa habilidades de trbajo.

## AHORA AGREGAMOS UNA HABILIDAD NUEVA

Para entender como es posible que la metodologia HK mejora su habilidad de estimar tiempos de nuestras practicas deliberdas utilizando los datos de nuestros habitos debemos entender un poco de estadistica, esite los grupos de un muestra que tiene relacion entre si y algunos tiene mas relacion y se pueden llamar que son causalares. Estos grupos que podemos decir como variable socultas son nuestros habitos. epuesto que hay radica una gran imporatnacia de lo que hagamos se ejecute plenemaente miren por ejemplo el animo es una estado que avees nos impide de trabjar y trbajar en ese estado da resultados pobres por lo que a medadi que vamos registrando mas nuestras practicas debemos incluir los habitos como parte ufundamental a la hora de entenre oruqe algunas cosas nos cuestan mas que otras.

En esta parte estamos confgurando nuestro HK para que tengamos tanto proyectos que registran las practicas deliebradss pero tambien proyectos orientados a nuestro bienestar coo persona. Por eso cuando escogemos una nueva habilidad para nuestro trello el aspecto fisico se puede compartir entre todas las personas pues tener unos habitos saludables como el deporte ayudan a la productividad de nuestras ejecuciones por lo que capaz si empezamos a relaizar el ejercicio del yoga para mantener un equilibrio de cuerpo mente y estar mas agiles en nuestro flow de concentracion. Es decir como un deportista que n tiene relaciones sexuales antes de sus campeonatos para tener prepradao todo tu hombri adentro. De gual manera para nuestroas pacticas deliberadas debemos tener un regimen de salud que tenga a nuestra mentay cuerpo aptos par las exigencias de haer estas practicas. Visto desde este punto de vista mas mecanico sobre nuestra naturaleza entendemos que tdo tiene una casusa y debemos ir al arazi del problema para atacar el problema general. Entonces podemos decir que para el cuerpo como para la mente existen diferentes remedios y creo que no fallo al decir que tambien necesitamos una habilidad para la mente como es el leer, porque hasta ahora ya tenemos dos habilideas que vienieron de a par, Que era la administraion para los asuntos que requieren hacerce de vez encuando pero hay qe hacerlas como comprar papal hegenico. Iba de la mano con escribir por que escribir una lista de compras es lo que nos lleva ser organizados. De igual manera para nuestra salud podes crear dos habilidades nuevas, que tal algo referido al cuerpo y que contenga las tecnicas que sean exlusivamente fisicas y otra habildiad como la de leer que tenga tenicas exclusivas al ejercicio de la mente. Por el momneto solo estamos abrazando la idea de que nuevas habilideades neecesitasmos pero mejor explicado estar en los siguiente subtitutlos corresopndientes a cada habilidad. Donde a media que te explico que tecnicas debemos aprender vamos  a ir explicando el uso de HK ya por segunda vez reforzando asi nuestro habito de utilizar esta maquina que registra nuestros habitos y practicas.

### FUERZA FISICA: TODO SOBRE EJERCICIOS

Para el nombre de habilidad general que abarque todo tipo de ejercicos deportes e nicluso pasos de bailes porque esta relacionado al cuerpo y su movimiento he decidio que fuerza fisica es un buen nombre y como es un nombre clave al igual que projecto J debo conservarlo de tal forma en todas las instacias donde necesite ponerlo. Es asi que podemos crear un nuevo tablero o board en el trello con esta habilidad llamda fuerza fisica y bueno le damos su respectivo decorado y sus nombre si es necesario aveces me gsta que tengan un orden asi que pongo un numero antes peroclaro esto implica que el nobreahora contien un numero antes del nombre porque es un nombre clave pero esta es la flexibilidad de escoger un nombre que podemos utilizar al mismo tiempo para darle un formato. Asi una vez adentro veremos que tenemos qe agregar columans pero solo tenemos un proyecto se va ver un poco solitario, por lo que tambien el la metodologia HK utilizamos la subdivision en un mismo grupo para esto uitlizamos la letra del proyecto este caso J y agregamos otro subtitlo en este caso vamos a poner saludos al sol, pero mira que vos pon las actividades que hagas como gymancia y natacion y ademas puedes dividilar aun mas si en natacion hacs un estilo una acluman apra cada estilo, Estos subdivisiones nos indican que pertecene a un proyecto mayor ademas de tener una basae para ello en nuestro libro digital por lo que no hay confusion, es de nuevo hacer uso de la flexibilidad de la metodolig auq e nos permite escoger el nombre eso si obviamente este es el nuevo nombre de nuestro proyecto y es identificador por lo que debe ir escrito igualen todas las demas itancias en especial en la de toggl.

Creada esta habilidad pasamos el lado facil por que debemos identificar las tecnias ue neeitamos para cumplir estos subgrupos. Es dificl por el aspecto que debemos darle un nombre que identifique la tecnica y sea de nuevo relativamente corta por ser un titulo identificador que utiliza el codigo de HK para el uso de estadisticas. Es un tema que veremos msa tarde pero por el moneto aprendermos a colocar tecnicas como templates ademas de saber identifiacar que cosas debemos poner en estos templates. Por que al ser ejercicios fisico entran en otro aspecto de las practicas deliberadas donde no se basa en la lectura sino en la ejecucion por lo que tener los pass escritos nos son de mucha utilidad en la ejecucion. Asi vemos que la metodolgia HK de neuvo tiene esta forma de registrar los habitos de una mera diferente a las practicas deiberadas. Podemos utilizar estas tecnicas como ejemplo de que ahcer cuando las practicas son de indole instanteno que registrar su tiempo de ejecucion es imposible por lo que debemos hallar otra forma de registrar un tiempo qe ademas significque o traiga consigo informacion de la ejecuion.Otros casos donde se pueden presentar estos es en la cocina, el canto. la cirijias , los diferentes tipos de arreglar algo. Y asi mcuhas haiideades caes en esta categoria de que un onjunto de tecnicas son utilizadas y que su trackeo indivual resulta imposible al entrometerse fallidaente con el flow de la ejecucion. Asi que veremeo ac ontinuacion como lidiar con estas situacion basados en unejemplo de ejercio de yoga.

#### pose de la moonta;a creo qe selllama

Ahora nos toca poner los templates de tecnica, poque hacemos de esta forma es poque al final de cuentas estas tecnicas las volveras a usar de una u otraforma por eso son tecnicas y mejor es utiliazr cada tecncai consucontexto. Pueto que ahora tenemos que hacer os saludos al sol, es obvio que debemos investigas y hacer uso e nuestro libro digital. Todo eso para llegar a tener el nombre de las tecnicas y vemos que el tal llamdo saludos al sol son un conjuntos de poses y cada pose tiene su nombre por lo que podemos decir que esa es la tecnica. Ahora vemos el problema como registrar un timepo de poses que su tiempo de ejecuuion se basa en respiracion. Para eso tomams lo siguiente mas grande que seria la practica de un circutio de todas las poses siendo una pose del sol completa . Esta peude ser nuestra medida auqnue primero continuemos con lo basico que es colocar el nombre de cada tempalte con el nombre de cada pose

En este paso utilizamos las mis metodologia pra colocar el titulo, es decir como es un template solo debe ir el #000 para el numero de uso, luego el nombre en mayusculas que seria la pose de la motna;a y luego una descripcion here para describir al hora del suo. Esta es su primer caractestica que todas deben compartir

#### luego viene de la monta;a pero saludando el cielo

Auqnue pasemos a la siguiente tarjeta es por el hecho que debo continuar explicando y espero que entiendan que cada atributo que estamos poniendo van en todos los templaes auqnue solo esten escritos en esta tarjeta que nos toca ahora que seria una variacin de la premra con un agregado. Denuevo su nombre debe estar en el mismo formtato #00 mas el nombre y con descrbire here para que luego al ser usasa el tempalte cambiemos ese descrbie here por el contexto donde esta siendo usada. De esta maenra lo siguiente que veremos que todas las tarjetas tempalte encesitan es la descripcion, ne este caso es un buen momento pra explicar coomo es el uso de esta tecnia como un recordatioro para cada ve que queiras volver. Toamndo en cuenta que todo lo que este debajo la division horizontal sera guardado y lo de encima se copiarda al calendario. Es decir el link al proyecto el titulo y la describcion donde se escirebne cada ejecucn del dia sno un template y seran cambiados luego pero se cambiaran con el tiempoemcbaio lo que queda abajao se perdura en el tiempo pero no se copia al calendario.

De esta forma debemos explicar en cada descripcion como se hace posible esta pose y las demas en sus decricoiones correspondietes, Tambien algun cnsejo que sepas y queiras agregas con el tiempo veras que este lugar en el temlate es tu bibliotecas de tecnicas entonces a mas tecnicas y mejor detallas tengas mas pdras navegasr y decidir cual te conveine . Y asi way.

#### depsues viene la de dejarse caer todo abajo con cabeza al suelo

Continado con la ocnfiguraciones de estos templaees es bueno mientras vamos actulizando los titulos y loas decricones de los nombres de las poses nos demos cuenta que estas tecnicas tan solo son pate de todo el yoga ashtanga por lo que es obvio que estamos tomando un camino de vanczar de apoco por que e yoga en suocnjunto ceunta con mas de 100 poses que se deben completar en hora y media y ese nivelal que queremos llegar empeicza desde abajo. Es decir que ahora tenemos que dominar el saludos al sol, pero cuando nos toque la parada de mano capaz dejemos de hacer saludos al sol para practicar mpradas de mano o quizas mezclremos las tecnicas para entar nuestra practica. Esta filosofia del yoga es tan aplicable en la metodolgoia que da sentido pues que la personalizacion es lo que hace que sea un practica fluida. De esta manera entendemos que cuando dejamos en el titulo un describe here y en la describcion un lugar de titulo y descriocion para cada ejecucon lo que queremos hacer es dejar un espacio para expresar el contexto denuestra practica para que se entienda de que manera estamos utilizando la tecnica.

Osea cuando hacemos uso de una tarjeta de template entra en la columa de nuestro poryectos en este caso deben estar debajo del proyecto de saludos al sol pero luego si tenemos otro ejercio que debemos hacer como salotos en cuerda es otro poryecto y emrece otros tempaltes de tecnica. Y si un dia decidimos utilizar una tecnica en un potyecto que era destianda a ser usada en otro proyecto puede salirnos cosas geniales como un saltarin de cuerda que hace poses de yoga mientras salta. Esto quiere decir que tenemos controlde las tecnicas que aplicamos y de como explicamos en el titulo y la descricion el contexto donde a lo largo sirve de explicativo de como lo hicemos funcnonar o recmonedacion de no volver a juntarlos. Sera como un material para leer cuando entre al abba;o.

#### despues viene la misma todo abajo pero levantando cabza lo maximo a frente

Entonces hay una peuqe;a diferencia a los templates que hciemos en la primera vez que utilziamos el trelo, esta vez estas tecnicas no tienens checjlist por que no se aplican en el tiempo sino qe son instantenas y difieren de cada contexto. Por lo que no es lo mismo y no merecen tener un checklist que solo es un detalle que na grega nada al flow mas que obstaculos. Es asi que en este caso las tecnicas son mas libros donde vuevles a tneder y tanbien pueden a lelgar a ser mas si tu personalizaicon creativamente lo permite pero en su base idiologica no necesita mas checklist. Esto no quiere decir que tenga passo apra realizarce pero es algunamaenra mejor tenerar descritas como pasos a cumplr que como checklist ya que como vimos antes los checklist de esta metodolgia son tres y agrupan ciertas cualidaes que no aplican aqui.

Ahora sin checklist todavia hace falta algunos caracteristicas mas, como el cover no es parte de a metodologia entonces puedes utlziarla si deseas y le vez uso. OTro es los label, pues estos siguen siendo algo importante pues tiene los dos accionadores de atuomtiacion que son los label emepzando y termiando por lo que si debes agreagarlos de la siguiente forma, Todos las tecnicas que vayas a utlizar en una ejecucion deben estar cooordinadas entre si para empezar igual ya que las contien un contexto. Esto es imprantate por que una vez que terminen su contexto pueden volver a empezar pero con otro contexto.

#### despues esta salto atras y cobra o perro cabeza arriba

Para tener un ejempllo de lo dicho podemos entender estas tecnicas como parte del yoga ashtanga osea que nosotros noestmaos aplicando al practica completa sino que nos vamos a enfocar n doinar estas poses par contnar con las iguente diifucltad y asi way.Etnonces en contexto todas estas tecnicas a pesar de ser del yoga ashtanga en este moento son del la practica de dominacion. Entonce ya tenemos el contexto pra nuestra tecnicas y donde dice describe here en cada ditutlo al la hora del uso le vamos a poner contexto, la ractiaca pra dominar los saludos al sol. D es forma cand emepcmeos haer esta actividad sera nuestro dia uno. Dediqueoms un pensmaiento mas en este atributo meintras continuamos creando los templates con sus nombres de cada pose

Cada pose es parte de una conunto que es el saludo y osotros debemos hacer 108 saludos, entonces podemos decir que el nombre del ejercio es 98 saudo al sol. para eso repetimos 108 vecs cada pose pero en una secuenciaia rigida que debemos cumplir. Esto ahce que debamos tomar estas tecnias que estmaos esribiendo como un conjunto que van a ir de la mano hasta quee lelguemos a las 108 seguidas. Ahora ya tenmos un conexto de empezar y uno de terminar. Puesto que sabemos que van ir juntas las tecnicas y eso es empezar el dia que ejcutamos ese ocnjunto es el dia uno y capaz ese dia solo lelguemo hacer 2 de las poses y ese es un dato que debe ir anotado en la describicon que va cambiando cn cada ejecucn con su titulo pegajoso obvio. El dia que termina estos ejerccions en conjunto sera el dia que lelguen a los 108. pero como sabemos que eso puede nunca llegar mejor acalrom una meta de completar 21 dias estos ejercicios para primero cumplir con la disciplina luego veremos el numero. Neuvamente elcontexto del conjunto de estas tenica ya tiene un nombre 21 dais de saludos al sol.

#### ahora levantamos cola y perro cabeza abajo

Esta maenra de agrupar tecnicas es como la mtodologia HK puede indentiafcar qe estas son tecnias que van en conjunto, de igual manera otras tencias va en conjunto y no puede ser trackeadas de forma indicvidual aportando informacionutl para las estadisicas es por eso que en conjunto rpesentan mas y mucas veces se basan en repetir varias veces un conjunto de tecnias para doinar el hacer algo, tomemos el ejemplo de cortar el cabello. la teoria peude ser muy corta pero en la practica existen varias tecnica para que el conjunto que es ejecutar un corte de pelo salga bueno que deben hacerce, Nose mepezando por la tecnica de tener todo afilado y capaz apsando po la tecncia de identificar el cabello de cleinte y esta tecnica debe tener varios detalles coom tips como de cuando vez pelos puntas paradas es este diagostico y cuando veaz que es esesequedas es este otro tip y todo esto va en tu trello en al descricon que no cambia despues de la divisoin horizontal.Tememos entonces que este conjunto de tecnias que se aplican en diferentes actividades no pueden ser trackeadas a menos que las tratemos como conjnto

Y al hacer que tenga un titulo identificador como nu nombre de la tecnica y ademas un detalle que llenamos a la hora de aplicar e bueno que entanos esto y convoquemos al conjunto con la misma descricpion pues que todas comparten el cpntexto, incluso sino es de forma literal pues deben asemejarse porque no hay interparatacion de parte del codigo sino qe es una interpretacion personal de com hacemos estos conjusnto. Seguro que las tecnias para patear n tiro libre para messi en su cabeza desde como empeiza a correr aasta en que lugar del baln le da son tecnicas qu el practica en forma de conjunto y mas imporante tiene su sello personal. Lo mismo queremos lograr tener la conciencia de que estas tecnicas hacen un conjunto y si ncontraom la receta perfecta podemos tener  un conjunto de tencias que nos permitan destacar en la eejcucion de ciertas habilaides.

#### del saludo b esta es la sentadatilla que nombre tendra

Ahora creo que podemos entender que amedia que fuimos haceindo estos remplates fui explicando como van a ser utilizados, este paso es como un tirual donde identificas  los nombres de las tencias y su contexto en que vaya a utilizar y sin son la primera vez que vas apoenrlos en la metodolga te tocara hacer el template correspondiente y mentras lo bacesy es lo que lo convierte en un ritual vas imaginando su uso en el futuro por lo que ya el conjunto del que nuestro cerebros agrupan las tecnicas para cumplir una accion se va formando abastanctamente ennuestra mente y conla practica deliberada que es el heco de que cada ve que ejecutemos este conjnto de tecnias vayamos ganando expertisie por lo que volvermols al trello actulizamos nuestros templaes.

De esta forma vamos entiendiendo la importancia de os templates porque son estas librerias de tecnias para la habildiad que podemos utilizar como queramos pero por el hehco de tenerlas organizadas y actulziadas con nuetrso descubiremientos experienias es qe van ganando imprtancia con elt miepo y ademas hacen que cada template maneje una sola tecnica por lo que es facil aplicar.

#### y del salubo b esta es lapose del guerrero

por ultimo eto es completo

consluiones de aca

conslusiones tambien de fuerza fisica nueva

Conslusines de la nueva habilidad qe exigimos

## OTRA HABILIDAD OPUESTA

Claro ahora otraexigencia de habiida

### ESTUDIAR: Y TODAS LAS TECNICAS QE CORRESPONDEN

Estudiar y todos sus subactegorias

#### cuando vemos videos de algo que queremos qprender

ahora solo pongo teniacas en trello

coclsuiones de este

#### cuando leemos un blog de algo queramos aprender

este es sobre un lugar

cnslusoines de este

#### cuando leermos un libro corto de algo queramos aprender

ahora sobre un libro rapido

consluones de este

#### cuando leemos un libro largo de algo dueramos aprender

otr tecnica qe escirbir

consluines de esta

#### cuando hacemos un get started de algo dueramos aprender

otr tecica mas de codgo

pero consluione de esta

#### cuando seguimos un tutorial pasos a pasos de algo dueramos aprender

tecinca de segir un tiorial

otro conslsiones

#### cuando fallamos en una tecnica hasta aprender

otr atecnica para aprender

conslusiones de esta tencia

#### cuando buscamos y filtramos links de internet que posiblemente ense;en algo

wao queda ua tencia mas

consluioesde una tencica

cpms;isopmes de tencua aprender aca

tenica de nueva ahbaiida en trello 

## TRACKEA EL TIEPO DIFERENTE

Super imporante aprender a tracker el tiempo

### AVECES SUCEDE SITUACIONES QUE REQUIEREN

en situaciones de segunods es beuno tener esta segunda opcion

#### es como el ejercicio es mejor cornometrar y luego subir el total

primero esta en tener un cronometro

consluiones

#### pero sin perder los datos entonces pueden ir en describcion

luego tener este importe

cosnisones de este

consiosnes de aveces sucede

conslusiones de trackeel tiempo diferente

## CONOCE TUS HABITOS REALES

claro ahora tengo qe poner el libro

### EN TU HK BOOK DEBES REPRESETAR TUS HABITOS

esta es tu ultima forma de darle un template pero tambien de personalizar

#### esto significa idintificar las acciones qe se pueden tomar datos

aca vien algo imporante

conslisiones de este

#### ademas significa tener la obligacion de hacerlos ilcuyendo no hacerlo

ibne otro obligancion

consluones de este

#### es parecido a un x cada dia pero aca lo iportante es identificarlos

biene un sdia parecido cada dia

cms;iospmes de ;p qe

#### con el fin de modificarlo a nuestro gusto

tenica de fin de modicar

conslusiones rapidas de esto como idea

consliosnes de tener habitos reales

conslusones de tu libro que se llama HK book

## MODIFICA TU CALENDAR TASK

ahora si ibne algo nuevo qehacr

### LO MAS IMPORTANTE ES QUE SABER QE QUIERES DE HABITO

so esto es imporante

#### lo qe no puedas dejar de hacer con lo qe debes hacer e tu rutina

aca viene algo qe portque

conslisones de este

#### Capaz lo mas imporatnte es qe veas como no la cumples

aca bien algo de porque

onslisones de este

#### sue;a con lo qe necesitas sin imposibles pero exigiendo

aca biene l bueno

conslsiones de este

conslisones de algo imporante

### VISUALIZANDO EN CALENDAR CON TASK

esto es imporante asi qe sigue como obligacion

#### claro no te dije qe se iba hacer solo pero es imorante

otro falma de hacer un paso

consliosnesde este

#### Coloca tu lista en los task por dia de las semna y repitelos

totro para so 2

consluosnes de paso 2

#### toma tiempos q sabes qe vas a cumplir para dar margen a modificar

vien algo mas qe bine

conslsioes de este paso

conslusones de vialauazilcando en calendario task

### UNA NUEVA RUTINA TACHEAR TUS LOGROS Y ANOTRAR

qie esto es las nueva rutina en tus dias dia aidia

#### cada dia escribis un cometario con el dato esto ira como dato de tu dia

poner en el calendario algo

consliosnes de como poner

#### cada dia techeas lo qe cumplist inslcu si no cumpliste

algo mas sencillo pero acordate

conslisoes de esto facl

consluones de una nueva rutuina de tachar

conslisones de todo el capito de modifica tu goole calendar task

---

CONCLUSIONES:Ahora vimos que agregando nuveas habilidaes nuestro trello crece y cada ve zson mas las cosas que debemos trackeaar y de eso se trata tener conciecnia pero seria mejor tener un dispositivo que nos ayuder como el cornometro peo integrado al toggle entonces los datos entrria automaticamente pero por el moento creo qe es lo qe hace de este habito algo qe esta dentro de su croazon mismo el tracker ca cada segundo los habitos y las tecnicas utilizadas didaid dida por que como dicen debemos explotar la tenclogia y si podemos agrega eventos de 30 segundo debemos hacer y asi igua acumulando en los datos mas variedad es decir tecnicas qe cada una lleva 30 seugnods hacerlas pero en todas se hacen como 10 min pero en separado por qe manejan un flow, pero en casos donde tengo que hacer 1000 de algo o tengo que estudiar 1000 paginas o debo lograr que funcione este prototipo que sigo de un tutorial podemos solo empezar a trackear y sin importar cuando poner finalizar y eso agregar como entry. claro aca  creo que no esta pero deberia agregar mas tiempos muertos al diario como 20 min de viaje de X a Y prque debende de un proyecto seguro y ademas es un registro pero e eso mas adelante,
